...
export async function generateStaticParams() {
    const pages = await getLocationsSlugs({ cat: 11 });
...
export default async function Location({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return (
        <main>
            ...
            <Header channel="baederwelten" />
            <ComponentRenderer content={location?.acf?.content} channel="baederwelten" locationData={location} />
            <Footer channel="baederwelten" />
        </main>
    );
}
