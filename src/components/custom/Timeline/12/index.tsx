...
type Content = {
    headline: string;
    text: string;
    boxes: { year: string; headline: string; text: string; image: Media }[];
    settings: Settings;
};
            ...
            <div className="container" ref={component}>
                <div className="relative">
                    <div className="absolute left-0 md:left-1/2 md:-translate-x-1/2 h-full w-px bg-blue">
                        <motion.div
                            style={{ height: height }}
                            className="absolute w-full h-0 bg-blue"
                        ></motion.div>
                    </div>
                    {content?.boxes?.map((step, index) => {
                        return (
                            <div
                                key={index}
                                className="grid md:grid-cols-2 gap-x-28 relative !mb-14 lg:mb-0 pl-8 sm:pl-14 md:pl-0"
                            >
                                ...
                                <div className={cn(index % 2 == 0 && "md:col-start-2")}>
                                    ...
                                    >
                                        <div className="p-6 sm:p-8">
                                            <div className="text-blue text-base leading-none mb-4">
                                                {step?.year}
                                            </div>
                                            <h4 className="text-h4 leading-small text-blue mb-6">
                                                {step?.headline}
                                            </h4>
