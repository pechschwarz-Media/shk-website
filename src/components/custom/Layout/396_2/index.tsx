        ...
        <div className="container">
          <div className="grid lg:auto-cols-fr gap-6 lg:grid-flow-col">
            {content?.boxes?.map((box, index) => {
              return (
                <div
                  className="bg-gray-medium p-8 rounded-xl flex flex-col justify-between"
                  key={index}
                >
                  <div>
                    <p className="text-boxNumber bg-blue text-white p-4 inline-flex rounded-xl leading-none mb-6">
                      {index + 1}
                    </p>
